...
const START_POINT = { x: 1969.0813, y: 3740.9275, z: 31.329712 };
const TOTAL_DROP_OFFS = 6;

export class Pizza2Job {
    /**
    ...
    static async begin(player: alt.Player) {
        const openSpot = await Pizza2Job.getVehicleSpawnPoint();
        if (!openSpot) {
            Athena.player.emit.notification(player, `~r~No room for vehicles right now. Please wait...`);
            return;
        }

        const randomPoints = Pizza2Job.getRandomPoints(TOTAL_DROP_OFFS);
