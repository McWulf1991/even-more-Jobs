...
const START_POINT = { x: -514.04834, y: -1019.3143, z: 21.44983 };
const TOTAL_DROP_OFFS = 2;

export class ConstdriverJob {
    /**
    ...
    static async begin(player: alt.Player) {
        const openSpot = await ConstdriverJob.getVehicleSpawnPoint();
        if (!openSpot) {
            Athena.player.emit.notification(player, `~r~No room for vehicles right now. Please wait...`);
            return;
        }

        const randomPoints1 = ConstdriverJob.getRandomPoints1(TOTAL_DROP_OFFS);
        ...
        const randomPoints2 = ConstdriverJob.getRandomPoints2(TOTAL_DROP_OFFS);
        ...
        const randomPoints3 = ConstdriverJob.getRandomPoints3(TOTAL_DROP_OFFS);
        ...
        const randomPoints4 = ConstdriverJob.getRandomPoints4(TOTAL_DROP_OFFS);

        for (let i = 0; i < randomPoints3.length; i++) {
            const rPoint4 = randomPoints3[i];
            objectives.push({
                description: 'Drop Off Supplies',
                type: JobEnums.ObjectiveType.WAYPOINT,
                pos: rPoint4,
        ...
        const randomPoints5 = ConstdriverJob.getRandomPoints5(TOTAL_DROP_OFFS);
        ...
        objectives.push({
            ...
            callbackOnFinish: (player: alt.Player) => {
                ...
                const earned = Math.floor(Math.random() * 250) + 250;
