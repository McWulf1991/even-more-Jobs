...
const START_POINT = { x: -509.45935, y: -1001.7099, z: 22.534058 };
const TOTAL_DROP_OFFS = 2;

export class ConstworkerJob {
    /**
    ...
    static async begin(player: alt.Player) {
        const openSpot = await ConstworkerJob.getStartingPoint();
        if (!openSpot) {
            Athena.player.emit.notification(player, `~r~No room for starting the job. Please wait...`);
            return;
        }

        const randomPoints1 = ConstworkerJob.getRandomPoints1(TOTAL_DROP_OFFS);
        ...
        const randomPoints2 = ConstworkerJob.getRandomPoints2(TOTAL_DROP_OFFS);
        ...
        const randomPoints3 = ConstworkerJob.getRandomPoints3(TOTAL_DROP_OFFS);
        for (let i = 0; i < randomPoints3.length; i++) {
            ...
            objectives.push({
                ...
                callbackOnFinish: (player: alt.Player) => {
                    const extra = Math.floor(distance3 * 0.1)
        ...
        const randomPoints4 = ConstworkerJob.getRandomPoints4(TOTAL_DROP_OFFS);
        ...
        const randomPoints5 = ConstworkerJob.getRandomPoints5(TOTAL_DROP_OFFS);
        ...
        const randomPoints6 = ConstworkerJob.getRandomPoints6(TOTAL_DROP_OFFS);
