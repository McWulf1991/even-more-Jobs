...
import { networkIsPlayerActive } from 'natives';

const START_POINT = { x: -513.2176, y: -51.17802, z: 41.10254 };
const TOTAL_DROP_OFFS = 5;

export class Mule2Job {
    /**
    ...
    static async begin(player: alt.Player) {
        const openSpot = await Mule2Job.getVehicleSpawnPoint();
        if (!openSpot) {
            Athena.player.emit.notification(player, `~r~No room for vehicles right now. Please wait...`);
            return;
        }

        const randomPoints = Mule2Job.getRandomPoints(TOTAL_DROP_OFFS);
        ...
        for (let i = 0; i < randomPoints.length; i++) {
            ...
            objectives.push({
                ...
                callbackOnStart: (player: alt.Player) => {
                    ...
                    Athena.player.emit.animation(player, `anim@mp_player_intmenu@key_fob@`, 'fob_click_fp', ANIMATION_FLAGS.UPPERBODY_ONLY | ANIMATION_FLAGS.ENABLE_PLAYER_CONTROL, -1);
                },
        ...
        objectives.push({
            ...
            callbackOnFinish: (player: alt.Player) => {
                ...
                const earned = Math.floor(Math.random() * 200) + 200;
